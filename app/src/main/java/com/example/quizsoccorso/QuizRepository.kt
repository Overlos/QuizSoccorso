...
class QuizRepository(
    ...
    suspend fun loadQuestions(): List<QuizQuestion> = withContext(Dispatchers.IO) {
        ...
        val (sanitized, remap) = ensureUniqueIds(questions)
        if (remap.isNotEmpty()) {
            migrateStats(remap)
            // Se abbiamo dovuto correggere gli ID al caricamento, salviamo subito la versione pulita
            saveQuestions(sanitized)
        }
        sanitized
    }
     ...
     */
    private fun ensureUniqueIds(questions: List<QuizQuestion>): Pair<List<QuizQuestion>, Map<Int, Int>> {
        val usedIds = mutableSetOf<Int>()
        val remap = mutableMapOf<Int, Int>()
        var nextId = (questions.maxOfOrNull { it.id } ?: 0) + 1
        
        val sanitized = questions.map { q ->
            if (q.id <= 0 || (q.id in usedIds)) {
                val oldId = q.id
                val newQ = q.copy(id = nextId++)
                usedIds.add(newQ.id)
                if (oldId > 0) remap[oldId] = newQ.id
                newQ
            } else {
                usedIds.add(q.id)
                q
        ...
        return sanitized to remap
    }

    private suspend fun migrateStats(remap: Map<Int, Int>) {
        val stats = loadStats().toMutableMap()
        var changed = false
        remap.forEach { (oldId, newId) ->
            stats.remove(oldId)?.let { stat ->
                stats[newId] = stat
                changed = true
            }
        }
    ...
    suspend fun importDatabase(uri: Uri): Int = withContext(Dispatchers.IO) {
        ...
        val (sanitized, remap) = ensureUniqueIds(combined)
        if (remap.isNotEmpty()) {
            migrateStats(remap)
        }
        saveQuestions(sanitized)
